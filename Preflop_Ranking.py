...
class PreflopHands:
    def __init__ (self):
        self.totalHands = 169
        self.allHands = np.zeroes((75, 75), dtype=int)
        self.fullDeck = Deck()
        self.rank(self)
        ...
        for firstCard in Deck:
            for secondCard in Deck:
                if firstCard.number <= secondCard.number:
                    self.allHands[firstCard.cardValue][secondCard.cardValue] = self.rank(card1 = firstCard, card2 = secondCard)
                else:
                    self.allHands[firstCard.cardValue][secondCard.cardValue] = self.rank(card1 = secondCard, card2 = firstCard)

    def rank (self, card1, card2):
        self.Hand = Hand(card1 = card1, card2 = card2)
        if self.Hand.suited:
            if card1.number == 13:
                if card2.number == 14:
                    return 4
